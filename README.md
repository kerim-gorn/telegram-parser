...
- **domains** (JSONB): Array of domain objects with subcategories:
  - `CONSTRUCTION_AND_REPAIR`: детализированные подкатегории (`TURNKEY_RENOVATION_CREWS`, `DESIGN_AND_PLANNING`, `APARTMENT_HANDOVER_INSPECTION`, `WALL_FINISHING_PLASTER_PAINT`, `BALCONY_AND_LOGGIA_WORKS`, `TILING_WORKS`, `WINDOWS_REPAIR_AND_ADJUSTMENT`, `PLUMBING_SERVICES`, `CUSTOM_FURNITURE`, `AIR_CONDITIONING_SERVICES`, `INTERIOR_DOORS_INSTALLATION`, `ELECTRICAL_WORKS`, `STRETCH_CEILINGS`, `FLOORING_SCREED_AND_LAMINATE`, `COTTAGE_AND_HOUSE_STRUCTURAL_WORKS`, `TOOLS_AND_MATERIALS`, `IRRELEVANT_WORKS_AND_QUESTIONS`)
