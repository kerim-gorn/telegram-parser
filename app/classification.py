...
Под интентом **REQUEST** мы понимаем **потенциальный лид** — сообщение, в котором:
...
Если пользователь **просто задаёт информационный вопрос**, делится опытом, обсуждает условия, задаёт уточнения к уже существующей услуге/сделке, жалуется или флиртует с идеей, но **не просит найти исполнителя/товар/конкретное решение для своей задачи**, это **НЕ REQUEST**, а чаще всего `INFO`, `COMPLAINT` или `OTHER`.

### Примеры по домену CONSTRUCTION_AND_REPAIR (стройка/ремонт)

**1. Настоящие REQUEST-лиды (должны быть REQUEST + CONSTRUCTION_AND_REPAIR / SERVICES):**

Такие сообщения нужно разметить как минимум:
- `intents` содержит `"REQUEST"`;
- `domains` содержит `{"domain": "CONSTRUCTION_AND_REPAIR", ...}` (или при необходимости `SERVICES` в дополнение);
- `is_spam = false`.
...
- «Доброе утро! Подскажите, пожалуйста, **хорошего Электрика**?)»
...
- Слова и конструкции «**ищу / нужен / нужна / нужны / посоветуйте / подскажите контакты / есть ли кто делает / есть у кого контакты / поделитесь контактами / можете порекомендовать**».
- Упоминание **конкретных ролей/услуг**: электрик, сантехник, ремонтная бригада, дизайнер, плиточник, потолки, натяжные потолки, мебель на заказ и т.п.
- Ожидание **ответа в виде контакта/исполнителя или конкретной рекомендации**.

**2. Сообщения, которые НЕ являются лидами (НЕ REQUEST, чаще всего INFO/OTHER):**

Даже если присутствуют слова «подскажите», «вопрос», «как», такие сообщения не про поиск исполнителя или услуги, а про информацию/обсуждение:
...
Для таких сообщений:
- **НЕ** ставить `REQUEST`, если нет явного/скрытого запроса найти исполнителя/услугу/товар.
- Чаще всего `intents = ["INFO"]` или `["OTHER"]`, или `["COMPLAINT"]` при жалобе.
- Домены выставлять по смыслу (например, OPERATIONAL_MANAGEMENT, SOCIAL_CAPITAL и т.п.), но **без REQUEST**, если нет лида.

### Исключения: что НЕ является REQUEST-лидом

**1. Неполные сообщения (часть мысли):**

Сообщения, которые выглядят как обрывок фразы без контекста, не являются REQUEST-лидами:
- Пример: «Кто то работу руками делает» — это часть мысли без запроса, просто констатация факта
- Такие сообщения → `intents = ["OTHER"]` или `["INFO"]`, **НЕ** `REQUEST`
- Если нет явного запроса услуги/товара/исполнителя → **НЕ** REQUEST

**2. Мелкие подработки (поиск 1-2 человек на простую физическую работу):**

Запросы на поиск "помощника", "работяги" на простую физическую работу без квалификации **НЕ являются REQUEST-лидами**.

Характерные признаки мелкой подработки:
- **Ключевые слова**: "помощник", "работяга", "на несколько часов", "на руки" (с указанием суммы)
- **Описание простой физической задачи**: "сложить", "перенести", "подать инструмент", "придержать", "сбить", "разбить", "расставить мебель"
- **Небольшая оплата**: обычно до 10-15к рублей
- **Размытое описание задачи** или намеренно простое дело
...
Для таких сообщений:
- Если это **предложение работы** → `intents = ["OFFER"]`
- Если это **запрос помощи** → `intents = ["INFO"]` или `["OTHER"]`
- **НЕ** ставить `REQUEST`, так как это не поиск квалифицированного исполнителя/услуги

**3. Мелкие стройматериалы/инструменты (используется подкатегория SMALL_TOOLS_AND_MATERIALS):**

Запросы на аренду/одолжить инструмент, куплю мелких стройматериалов **ЯВЛЯЮТСЯ REQUEST**, но относятся к подкатегории `SMALL_TOOLS_AND_MATERIALS`:
- `intents = ["REQUEST"]`
- `domains = [{"domain": "CONSTRUCTION_AND_REPAIR", "subcategories": ["SMALL_TOOLS_AND_MATERIALS"]}]`

Примеры (REQUEST с SMALL_TOOLS_AND_MATERIALS):
...
### Общие правила по всем доменам

1. **REQUEST = лид**:
   - Пользователь **хочет получить** услугу, товар, рекомендацию исполнителя, помощь специалиста, консультацию для решения своей задачи.
   - Есть **целевая потребность**: «ищу няню», «нужен юрист», «посоветуйте врача», «подскажите, где заказать шкаф», «кого посоветуете для…».

2. **НЕ REQUEST**:
   - Просто **уточнение информации**: условия, высота потолков, особенности дома, правила УК, детали ДДУ, как что-то устроено.
   - **Обсуждение опыта**, вопросов эксплуатации, правовых нюансов без запроса конкретного исполнителя/услуги.
   - **Жалобы** (`COMPLAINT`) без просьбы «порекомендуйте, к кому обратиться».
   - **Социальное общение, новости, оффтоп**, бюро находок и т.п.
   - **Скам/спам** — отмечать `is_spam = true` и не считать это полезным REQUEST-лидом.
   - **Неполные сообщения** — обрывки фраз без контекста, часть мысли без запроса.
   - **Мелкие подработки** — поиск 1-2 человек на простую физическую работу без квалификации (помощник, работяга на несколько часов, простые задачи типа "сложить", "перенести", "подать инструмент" с небольшой оплатой до 10-15к).

3. Если сообщение **на границе** между `REQUEST` и `INFO`:
   - Если нет явного запроса контактов/исполнителя/услуги — лучше отнести к **INFO/OTHER**, **не** к REQUEST.
   - Модель должна быть **консервативной**: лучше пропустить сомнительное сообщение, чем размечать как REQUEST то, что не является настоящим лидом.
   - При сомнениях между REQUEST и мелкой подработкой — выбирать INFO/OFFER/OTHER, **не** REQUEST.

4. **Multi-labeling по доменам**:
   - Сообщение может относиться к нескольким доменам (например, ремонт + услуги, право + недвижимость).
   - Важно, чтобы при наличии REQUEST хотя бы один домен отражал **предмет запроса** (например, CONSTRUCTION_AND_REPAIR для запроса ремонтной бригады).

5. **Reasoning:**:
   - Будь краток. Объясни, почему выбран именно такой интент и срочность (например: «Явно ищет ремонтную бригаду — это лида REQUEST в стройке» или «Только уточняет высоту потолков, без поиска исполнителя — это INFO»).
...
[
  ...
  {"id": "7", "text": "Сложить кирпичи на поддоны — 5300₽ и докину на дорогу домой."}
]
...
class DomainInfo(BaseModel):
    ...
    subcategories: List[str] = Field(
        ...
        description=(
            ...
            "   - MAJOR_RENOVATION: Крупный ремонт - один из первых этапов в ремонте квартиры или строительстве дома, строительные работы значительного объема и времени с большим чеком\n"
            "   - REPAIR_SERVICES: Ремонтные услуги - стяжка пола, услуги плиточника или маляра, установка окон, потолки, приемка квартиры\n"
            "   - SMALL_TOOLS_AND_MATERIALS: Мелкие стройматериалы и инструменты - аренда/одолжить инструмент (пылесос, тепловизор, сверло), купля/продажа мелких стройматериалов (гипсокартон, двери от застройщика), мелкие услуги (установка одной двери, вывоз мусора)\n"
