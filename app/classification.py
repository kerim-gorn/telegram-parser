...
11=REPUTATION, 12=IMMIGRATION_AND_VISAS, 13=NONE
...
7 AUTO: 1=AUTO_PURCHASE, 2=AUTO_PREMIUM_DETAILING, 3=AUTO_REPAIR, 4=AUTO_SERVICE_STATION, 5=AUTO_INFO_QA, 6=AUTO_RENTAL
...
1 REQUEST: Под интентом REQUEST мы понимаем потенциальный лид — сообщение, в котором:
    ...
    2) На такое сообщение уместно и содержательно ответить именно рекомендацией: дать контакт мастера, компании, врача, юриста, репетитора и т.п. Если уместный ответ — только поделиться опытом, информацией, статусом («мы уже подали заявку», «мы соседи с участка X», «да, у нас такая же ситуация»), без передачи контакта исполнителя/компании, то это НЕ REQUEST.
...
7. AUTO:
   1. AUTO_PURCHASE: Покупка автомобиля — подбор машины, пригон, сделка купли-продажи, поиск продавца/перекупа как покупатель
   ...
   3. AUTO_REPAIR: Ремонт автомобиля — замена деталей, кузовной ремонт, двигатель, трансмиссия, тормоза, подвеска, автоэлектрика
   4. AUTO_SERVICE_STATION: СТО, шиномонтаж и мелкие работы по авто; «прикурить» / эвакуация; одолжить бустер для запуска двигателя (код в выводе строго AUTO_SERVICE_STATION, не используй другие имена)
   5. AUTO_INFO_QA: Вопросы про комплектации, пакеты опций, отличия модификаций, нейтральные вопросы владельцам без явного намерения купить машину сейчас
   6. AUTO_RENTAL: Аренда легкового автомобиля — прокат, долгосрочная аренда, каршеринг, поиск/сравнение сервисов аренды (не недвижимость)
8. MARKETPLACE:
...
12. IMMIGRATION_AND_VISAS: Получение и продление виз, ВНЖ и миграционных статусов, помощь с оформлением международных документов (переводы, апостиль, консульские справки и др.)
13. NONE: нет подходящего домена"

- Сообщение может относиться к нескольким доменам.
- При наличии REQUEST хотя бы один домен должен отражать предмет запроса (услуга, товар, консультация или контакт исполнителя).
- Если домен не подходит — ставь 12 (NONE) и только его.
- Домен и подкатегории ставь только тогда, когда в тексте есть запрос или предложение, связанное с этим доменом (например, пользователь хочет заказать услугу, купить/снять объект, получить консультацию). Простое упоминание темы («участок», «забор», «электричество», «ремонт», «квартира», «дом» и т.п.) без запроса/предложения не даёт основания выставлять домен.
...
==================================================
6.1) Настоящие REQUEST-лиды (REQUEST + CONSTRUCTION_AND_REPAIR) по подкатегориям (НЕ СПАМ):
    - 1. TURNKEY_RENOVATION_CREWS: Бригады для черновых и отделочных работ, ремонт под ключ в домах, квартирах и студиях.
...
6.2) Подкатегория IRRELEVANT_WORKS_AND_QUESTIONS – очень мелкие работы – какие-то задачи на пару часов, которые стоят незначительных средств; вопрос может касаться дорогостроящих услуг, но люди явно просят совет, как им самим решить задачу без надобности в привлечении других людей. Сообщения данной подкатегории ЗАПРЕЩЕНО помечать как REQUEST (INFO/OTHER/COMPLAINT).
    Примеры сообщений:
...
==================================================
7) AUTO DOMAIN DETAILS
==================================================
7.0) Общие правила для домена 7 (AUTO)
- Ставь домен AUTO только если сообщение по смыслу про легковой автомобиль / владение / покупку / сервис / аренду авто (не про «авто» как случайное слово в другой теме).
- Подкатегорию указывай только если она явно следует из текста; если ясно «про машины», но нельзя надёжно выбрать подкатегорию — domains=7, поле subcats оставь пустым.
- Не путай подкатегорию AUTO_RENTAL с доменом RENTAL_OF_REAL_ESTATE: аренда квартиры/дома.

7.1) AUTO_PURCHASE (код 1) — настоящие лиды на покупку (REQUEST), если НЕ спам:
- НЕ ставь подкатегорию 1 (AUTO_PURCHASE), если пользователь только уточняет комплектацию, сравнивает пакеты, спрашивает «что лучше взять» без поиска продавца/подрядчика на сделку — это AUTO_INFO_QA (код 5), intent обычно INFO или OTHER, не REQUEST.

7.2) AUTO_INFO_QA (код 5) — вопросы и обсуждения без лида на покупку:
    - Сравнение комплектаций, «что входит в пакет Комфорт», отличия моделей, расход, надёжность мотора, «у кого какая комплектация»
    - «Кто уже забрал машину, поделитесь впечатлениями» без просьбы контакта продавца
    - Координация владельцев одной марки без запроса купли-продажи
- Intent: чаще INFO или COMPLAINT (если жалоба на дилера/гарантию); REQUEST только если явно просят контакт именно для покупки — тогда уже AUTO_PURCHASE.

7.3) AUTO_RENTAL (код 6) — аренда легкового авто:
    - «Посоветуйте прокат авто на месяц рядом с ЖК»
    - «Ищу каршеринг / долгосрочную аренду, кто пользовался — отзовитесь»
- Если речь только про аренду без покупки — не используй код 1 (AUTO_PURCHASE).

7.4) AUTO_SERVICE_STATION (код 4), AUTO_REPAIR (3), AUTO_PREMIUM_DETAILING (2)
- Код 4 (AUTO_SERVICE_STATION): шиномонтаж, сезонная смена резины, мелкий ремонт на СТО, эвакуатор, «прикурить».
- Код 3: крупный ремонт агрегатов, кузов, ДВС как в описании подкатегории 3.
- Код 2: премиальный детейлинг, оклейка плёнкой, дорогая покраска.

7.5) Подкатегория AUTO_INFO_QA и REQUEST
- Сообщения про «посоветуйте что выбрать из двух комплектаций», «что входит в …» ЗАПРЕЩЕНО классифицировать как REQUEST+AUTO_PURCHASE. Это INFO (или OTHER) + 7=5 (AUTO_INFO_QA), если домен AUTO уместен; если тема машины вторична — можно domains=13 (NONE).
...
2) Координация и инфо-вопросы между соседями (без запроса исполнителя):
- Сообщения, в которых пользователь знакомится, уточняет, есть ли соседи/владельцы участка/квартиры, спрашивает, «кто уже подал заявку», «кто сталкивался с такой же ситуацией», обсуждает общий статус, но НЕ просит контакты мастера/компании/подрядчика, относятся к INFO (или OTHER) и НЕ являются REQUEST.
- В таких случаях обычно ставится домен 13 (NONE), если нет явно выраженного запроса на услугу/товар/консультацию.
  Пример: «Здравствуйте, меня зовут Лидия, хотелось узнать есть хозяева 12 и 14 участка, у меня 13 участок, планируем ставить летом забор, подали заявку на электричество, может по нашей линии уже кто-то тоже подал заявку, т.к. у нас нет столбов совсем» → intents = INFO, domains = 13 (NONE).

3) Мелкие подработки являются спамовыми рассылками (поиск 1–2 человек на простую физическую работу):
Признаки:
...
==================================================
9) FEW-SHOT BATCH EXAMPLE (bitwise output)
==================================================
User Input:
[
  ...
  {"id": "7", "text": "Здравствуйте, меня зовут Лидия, хотелось узнать есть хозяева 12 и 14 участка, у меня 13 участок, планируем ставить летом забор, подали заявку на электричество, может по нашей линии уже кто-то тоже подал заявку, т.к у нас нет столбов совсем)))"},
  {"id": "8", "text": "Подскажите, в комплектации Престиж что входит в мультимедиа, есть ли CarPlay?"},
  {"id": "9", "text": "Ищу нормальный прокат авто на месяц, посоветуйте сервис в личку."},
  {"id": "10", "text": "Посоветуйте проверенного перекупа, хочу купить кроссовер до 4 млн."}
]
...
4|2|1||1|1|Мелкая подработка, не лид
5|6|12||0|1|Обрывок фразы
6|1|1|1=16|0|2|Просит одолжить инструмент
7|5|13||0|2|Координация с соседями
8|5|7|7=5|0|1|Вопрос про комплектацию
9|1|7|7=6|0|3|Ищет аренду авто
10|1|7|7=1|0|3|Лид на покупку авто
"""
...
class DomainType(str, Enum):
    ...
    IMMIGRATION_AND_VISAS = "IMMIGRATION_AND_VISAS"
    NONE = "NONE"
...
DOMAIN_CODE_TO_VALUE: dict[int, DomainType] = {
    ...
    12: DomainType.IMMIGRATION_AND_VISAS,
    13: DomainType.NONE
}
