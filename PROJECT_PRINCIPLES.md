...
* ❌ **НИКОГДА** не используй pyrogram, python-telegram-bot или другие библиотеки кроме Telethon
* ❌ **НИКОГДА** не создавай файлы `*.session`
* ❌ **НИКОГДА** не храни credentials в коде
* ✅ **ВСЕГДА** используй async/await для I/O операций
* ✅ **ВСЕГДА** проверяй FloodWait ошибки и обрабатывай их корректно

## 5. СТРУКТУРА МОДУЛЕЙ

```
src/
├── db/              # SQLAlchemy модели, подключения, CRUD операции
├── telegram_client/ # Telethon клиент, менеджер сессий, обертки API
├── workers/         # Celery tasks и Real-time воркер
├── settings/        # Pydantic Settings, загрузка .env
└── utils/           # Декораторы (flood_wait), логирование, хелперы
```

## 6. WORKFLOW РАЗРАБОТКИ

1. **Настройка окружения:** `.env` → docker-compose up
2. **Миграции БД:** Alembic для всех изменений схемы
3. **Тестирование:** pytest с asyncio плагином
4. **Деплой:** Docker Compose на продакшн

## 7. ANTI-BAN СТРАТЕГИЯ

* Exponential backoff при FloodWait
* Случайные задержки между запросами (jitter)
* Ограничение количества сообщений за батч
* Логирование всех rate-limit событий

## 8. ХРАНЕНИЕ ДАННЫХ

* **PostgreSQL:** Сообщения, метаданные чатов, пользователи
* **Redis:** 
  - StringSession (зашифрованные)
  - Кеш (опционально)
  - Celery результаты
* **RabbitMQ:** Очередь задач для парсинга и обработки

---

**Следуй этим принципам на КАЖДОМ этапе разработки. Любое отклонение должно быть явно обосновано.**
